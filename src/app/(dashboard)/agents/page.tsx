...
const INITIAL_AGENTS: Agent[] = [
  { id: 1, name: "My Agent", createdBy: "Yash Gupta", createdAt: "Apr 8, 2026, 6:17 PM" },
];
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-16 py-20">
      ...
      <div className="flex items-end justify-between mb-20 border-b border-border pb-8">
        <div>
          <h1 className="text-7xl font-semibold tracking-tighter leading-none text-foreground mb-2">Agents</h1>
          <p className="text-base text-muted-foreground tracking-tight">Manage your AI call agents</p>
        </div>
      ...
      {showBanner && (
        <div className="bg-card border border-border p-12 relative mb-20">
          <button onClick={() => setShowBanner(false)} className="absolute right-6 top-6 text-muted-foreground hover:text-foreground transition-colors">
            <X className="w-5 h-5" strokeWidth={1.5} />
          </button>
          <h2 className="text-3xl font-semibold tracking-tighter mb-3 text-foreground">Begin with a persona</h2>
          <p className="text-base text-muted-foreground mb-8 tracking-tight leading-relaxed max-w-2xl">Pre-configured agent templates to accelerate your workflow</p>
      ...
      <div className="mb-12">
        <div className="relative mb-6">
          <Search className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" strokeWidth={1.5} />
          <input
            type="text"
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-border">
              <th className="py-4 font-mono text-xs uppercase tracking-widest text-muted-foreground w-[40%]">Name</th>
              <th className="py-4 font-mono text-xs uppercase tracking-widest text-muted-foreground w-[30%]">Created by</th>
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr>
                <td colSpan={4} className="py-20 text-center text-muted-foreground text-base tracking-tight">No agents found.</td>
              </tr>
            )}
            {filtered.map(agent => (
              <tr key={agent.id} className="border-b border-border hover:bg-muted transition-colors group cursor-pointer">
                <td className="py-5 font-semibold text-foreground text-lg tracking-tight">{agent.name}</td>
                <td className="py-5 text-base text-muted-foreground tracking-tight">{agent.createdBy}</td>
                <td className="py-5 text-base text-muted-foreground tracking-tight font-mono text-sm">{agent.createdAt}</td>
