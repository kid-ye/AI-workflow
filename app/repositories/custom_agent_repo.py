...
class CustomAgentRepository:
    def create(self, db: Session, data: CustomAgentCreate) -> CustomAgent:
        agent = CustomAgent(id=str(uuid.uuid4()), **data.model_dump())
        db.add(agent)
        db.commit()
        db.refresh(agent)
        return agent
    ...
    def get_by_agent_id(self, db: Session, agent_id: str) -> List[CustomAgent]:
        return db.query(CustomAgent).filter(CustomAgent.agent_id == agent_id).all()
